...
const ProblemDescription:React.FC<ProblemDescriptionProps> = ({problem,_solved}) => {
	...
	const handleLike = async () => {
		if(!user){
			toast.error("You must be logged in to like this problem", {position:"top-left",theme:"dark"});
			return;
		}
		if(updating) return;
		setUpdating(true);
		...
		await runTransaction(firestore, async(transaction) => {
			...
			if(userDoc.exists() && problemDoc.exists()){
				if(liked){
					// Remove problem id from liked problems on user document.
				...
				<div className='px-5'>
					...
					<div className='w-full'>
						...
						<div className='text-white text-sm my-3'>
							{/* We use dangerouslySetInnerHTML because problem.problemStatement is already in HTML formal
							and we have to render it directly here (including HTML) */}
	...
	useEffect(() => {
		...
		const getCurrentProblem = async() => {
			...
			if(docSnap.exists()){
				...
				// console.log(problem,"Current problem is there!");
...
function useGetUsersDataOnProblem(problemId: string){
	...
	useEffect(() => {
		const getUsersDataOnProblem = async () => {
			...
			if(userSnap.exists()){
				// Getting array from firestore
				const data = userSnap.data();
				const {solvedProblems,likedProblems,dislikedProblems,starredProblems} = data;
				setData({
					liked:likedProblems.includes(problemId), // LikedProblems is an array which contains all problems liked by user.
